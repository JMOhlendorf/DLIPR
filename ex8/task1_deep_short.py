...
x_shortcut = Conv2D(8, kernel_size = (3,3), padding='same', activation='relu', name = 'encoder_conv_2')(x)
x = MaxPooling2D(pool_size=(2,2), padding='same')(x_shortcut)
...
model.fit(x_train_noisy, x_train, batch_size=8, epochs=20, verbose=2)
model.save('model_shortcut_binary.h5')

out_images = model.predict(x_test_noisy)
'''
model = models.load_model('model_shortcut_mse.h5')  # load your trained model -> you only have to train the model once!
model.summary()
out_images = model.predict(x_test_noisy)
#PLOTTING THE DATA
...
plt.savefig('reconstruction_deep_autoencoder_with_shortcuts_mse_test.png')
